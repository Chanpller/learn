...
```
[root@iZ2vcgkzipyks59pqqd6ewZ redis-8.6.2]# make && make install for dir in src; do make -C $dir all; done make[1]: Entering directory '/root/redis-8.6.2/src' CC Makefile.dep CC threads_mngr.o In file included from server.h:57, from threads_mngr.c:16: zmalloc.h:30:10: fatal error: jemalloc/jemalloc.h: No such file or directory #include <jemalloc/jemalloc.h> ^~~~~~~~~~~~~~~~~~~~~ compilation terminated. make[1]: *** [Makefile:478: threads_mngr.o] Error 1 make[1]: Leaving directory '/root/redis-8.6.2/src' make: *** [Makefile:14: all] Error 2

这是一个 Redis 编译错误，缺少 jemalloc 库。我来帮您解决这个问题。
问题原因： Redis 默认使用 jemalloc 作为内存分配器，但您的系统没有安装 jemalloc 开发库。

yum install -y jemalloc-devel

如果您不想安装 jemalloc，可以改用标准的 libc 内存分配器：
