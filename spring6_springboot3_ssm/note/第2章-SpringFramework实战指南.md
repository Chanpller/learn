...
- 单一架构
  ...
  ![img](C:\workspace\ideaProjects\learn\spring6_springboot3_ssm\image\2-1.png)
...
- 分布式架构
  ...
  ![](C:\workspace\ideaProjects\learn\spring6_springboot3_ssm\image\2-2.png)
  ...
  ### 2.1.2 框架概念和理解
    ...
    ![](C:\workspace\ideaProjects\learn\spring6_springboot3_ssm\image\2-3.png)
  ...
  ![img](C:\workspace\ideaProjects\learn\spring6_springboot3_ssm\image\2-4.png)
...
## 2.2 pringFramework介绍

  ### 2.2.1 Spring 和 SpringFramework概念
...
​    QQ = SpringFramework
  ...
  ### 2.2.2 SpringFramework主要功能模块

​    SpringFramework框架结构图：

![img](C:\workspace\ideaProjects\learn\spring6_springboot3_ssm\image\2-5.png)
...
​    ![img](C:\workspace\ideaProjects\learn\spring6_springboot3_ssm\image\2-6.png)
​    整个项目就是由各种组件搭建而成的：

 ![img](C:\workspace\ideaProjects\learn\spring6_springboot3_ssm\image\2-7.png)
...
![img](C:\workspace\ideaProjects\learn\spring6_springboot3_ssm\image\2-8.png)
...
![img](C:\workspace\ideaProjects\learn\spring6_springboot3_ssm\image\2-9.png)
...
#### 2.3.2.2 SpringIoC容器介绍

Spring IoC 容器，负责实例化、配置和组装 bean（组件）。容器通过读取配置元数据来获取有关要实例化、配置和组装组件的指令。配置元数据以 XML、Java 注解或 Java 代码形式表现。它允许表达组成应用程序的组件以及这些组件之间丰富的相互依赖关系。



![img](C:\workspace\ideaProjects\learn\spring6_springboot3_ssm\image\2-10.png)

上图显示了 Spring 容器工作原理的高级视图。应用程序类与配置元数据相结合，您拥有完全配置且可执行的系统或应用程序。

#### 2.3.2.3 SpringIoC容器具体接口和实现类
...
![image-20241023200316398](C:\workspace\ideaProjects\learn\spring6_springboot3_ssm\image\image-20241023200316398.png)
...
![image-20241023200413558](C:\workspace\ideaProjects\learn\spring6_springboot3_ssm\image\image-20241023200413558.png)

#### 2.3.2.4 SpringIoC容器管理配置方式
...
![image-20241023200625603](C:\workspace\ideaProjects\learn\spring6_springboot3_ssm\image\image-20241023200625603.png)

  ### 2.4.1 Spring IoC / DI 实现步骤
...
#### 2.4.2.1 实验一： 组件（Bean）信息声明配置（IoC）
   ...
   2. 思路
      ...
      ![img](C:\workspace\ideaProjects\learn\spring6_springboot3_ssm\image\2-11.png)
...
```
   ...
   - xml配置文件编写	
     ...
     ![img](C:\workspace\ideaProjects\learn\spring6_springboot3_ssm\image\2-12.png)
     编写配置文件：
...
```
   ...
   - factory-method:  指定实例工厂方法名。注意，实例方法必须是非static的！
     7. 图解IoC配置流程
        ![img](C:\workspace\ideaProjects\learn\spring6_springboot3_ssm\image\2-13.png)

#### 2.4.2.2 实验二： 组件（Bean）依赖注入配置（DI）
...
![img](C:\workspace\ideaProjects\learn\spring6_springboot3_ssm\image\2-14.png)
...
#### 2.4.2.3 实验三： IoC容器创建和使用

    1. 介绍
       上面的实验只是讲解了如何在XML格式的配置文件编写IoC和DI配置！
        如图：
       ![img](C:\workspace\ideaProjects\learn\spring6_springboot3_ssm\image\2-15.png)
...
#### 2.4.2.4 实验四： 高级特性：组件（Bean）作用域和周期方法配置

![image-20241023222727455](C:\workspace\ideaProjects\learn\spring6_springboot3_ssm\image\image-20241023222727455.png)
...
<!--init-method 初始化方法名-->
    ...
    <bean id="beanOne" class="com.chanpller.ioc.BeanOne" init-method="init" depends-on="beanTwo" />
    <bean id="beanTwo" class="com.chanpller.ioc.BeanTwo" init-method="init" destroy-method="cleanup" />
```

```
depends-on="beanTwo" 输出结果：
BeanTwo调用构造方法了
BeanTwo初始化了
BeanOne调用构造方法了
BeanOne初始化了
...
生命周期总结：如果没有依赖关系，调用了每个类的构造器后，再调用每个类的初始化方法。如果有依赖关系，会先调用依赖的类构造器，及依赖类的初始化方法
...
#### 2.4.2.5 实验五： 高级特性：FactoryBean特性和使用

![image-20241023224803273](C:\workspace\ideaProjects\learn\spring6_springboot3_ssm\image\image-20241023224803273.png)
...
​      ![img](C:\workspace\ideaProjects\learn\spring6_springboot3_ssm\image\2-16.png)

            2. FactoryBean使用场景
               - 代理类的创建
                 - 第三方框架整合
                   - 复杂对象实例化等（比如MyBatis框架整个）
               - Factorybean应用
                 - 准备FactoryBean实现类
...
```
结果
happyMachine = com.chanpller.ioc.HappyMachine@16ec5519
bean = com.chanpller.ioc.HappyFactoryBean@2f7298b
```
...
BeanFactory是 Spring 框架的基础，其作为一个顶级接口定义了容器的基本行为，例如管理 bean 的生命周期、配置文件的加载和解析、bean 的装配和依赖注入等。BeanFactory 接口提供了访问 bean 的方式，例如 getBean() 方法获取指定的 bean 实例。它可以从不同的来源（例如 Mysql 数据库、XML 文件、Java 配置类等）获取 bean 定义，并将其转换为 bean 实例。同时，BeanFactory 还包含很多子类（例如，ApplicationContext 接口）提供了额外的强大功能。

总的来说，FactoryBean 和 BeanFactory 的区别主要在于前者是用于创建 bean 的接口，它提供了更加灵活的初始化定制功能，而后者是用于管理 bean 的框架基础接口，提供了基本的容器功能和 bean 生命周期管理。

![image-20241023225520521](C:\workspace\ideaProjects\learn\spring6_springboot3_ssm\image\image-20241023225520521.png)

#### 4.2.6 实验六： 基于XML方式整合三层架构组件

    1. 需求分析
       搭建一个三层架构案例，模拟查询全部学生（学生表）信息，持久层使用JdbcTemplate和Druid技术，使用XML方式进行组件管理！
        ![img](C:\workspace\ideaProjects\learn\spring6_springboot3_ssm\image\2-17.png)
  2. 数据库准备
...
```
  ...
  ### 2.4.3 基于 注解 方式管理 Bean

#### 2.4.3.1 实验一： Bean注解标记和扫描 (IoC)
...
![img](C:\workspace\ideaProjects\learn\spring6_springboot3_ssm\image\2-18.png)
...
#### 2.4.3.2 实验二： 组件（Bean）作用域和周期方法注解 
...
#### 2.4.3.3 实验三： Bean属性赋值：引用类型自动装配 (DI)
...
```
     ...
     2. 工作流程
        ![img](C:\workspace\ideaProjects\learn\spring6_springboot3_ssm\image\2-19.png)    
...
#### 2.4.3.4 实验四： Bean属性赋值：基本类型属性赋值 (DI)
...
####  2.4.3.5 实验五： 基于注解+XML方式整合三层架构组件

      1. 需求分析
​    搭建一个三层架构案例，模拟查询全部学生（学生表）信息，持久层使用JdbcTemplate和Druid技术，使用XML+注解方式进行组件管理！
​           ![img](C:\workspace\ideaProjects\learn\spring6_springboot3_ssm\image\2-20.png)
​          2. 数据库准备
...
![img](C:\workspace\ideaProjects\learn\spring6_springboot3_ssm\image\2-21.png)
​    

#### 2.4.4.2 实验一：配置类和扫描注解
...
#### 2.4.4.6 实验五：基于注解+配置类方式整合三层架构组件

![image-20241023232823899](C:\workspace\ideaProjects\learn\spring6_springboot3_ssm\image\image-20241023232823899.png)

      1. 需求分析
​    搭建一个三层架构案例，模拟查询全部学生（学生表）信息，持久层使用JdbcTemplate和Druid技术，使用注解+配置类方式进行组件管理！
​          ![img](C:\workspace\ideaProjects\learn\spring6_springboot3_ssm\image\2-22.png)
​          2. 数据库准备
...
```
    ...
    4. 声明带日志接口实现
        ...
        ![](https://secure2.wostatic.cn/static/rP27XVMnTD1FtCx1maiUuW/aopimg002.png)
...
```
  ...
  ### 5.2 解决技术代理模式
    ...
    1. **代理模式**
        ...
        ![](https://secure2.wostatic.cn/static/2KdCVRG7sTJfhNGH3QaB58/aopimg004.png)
        
        有代理场景：
        
        ![](https://secure2.wostatic.cn/static/atPYpsBfyndTrgArisWrag/aopimg005.png)
...
​        
    ...
    JDK动态代理技术实现（了解）
      ...
      ![](https://secure2.wostatic.cn/static/23QMoxyYUoz7qSJeAnRUug/aopimg003.png)
...
​        
    ...
    ![](https://secure2.wostatic.cn/static/q1FA8t8MXYLLqk6gybZ5Ya/aopimg007.png)
...
​    
        ...
        ![](https://secure2.wostatic.cn/static/p2kNLy4hrpYoHRjpanrS2t/aopimg008.png)
        ...
        ![](https://secure2.wostatic.cn/static/ssFYqSKwRkku7yWm1KrHFo/apoimg010.png)
        ...
        ![](https://secure2.wostatic.cn/static/8Y4kgKdeMeNz2RSFkHo4o1/aopimg009.png)
...
​      ![](https://secure2.wostatic.cn/static/pRzPnt5i7zeasX3V3zYZQ4/aopimg006-1716953659294.png)
​    
...
```
    ...
    #### 5.5.4 切点表达式语法
      1. **切点表达式作用**
          ...
          ![](https://secure2.wostatic.cn/static/bYuoZYrB1bTE9phRx5AJfk/apoimg028.png)
            2. **切点表达式语法**
    
          切点表达式总结
        
          ![](https://secure2.wostatic.cn/static/u3zGFYMEQBEc9aXovEQo1R/apoimg011.png)
