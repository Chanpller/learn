...
1. 对象在`JVM`中是怎么存储的？
   ...
   * 对象头
     ...
     * 运行时元数据（Mark Word），包括哈希值（hashcode）、GC分代年龄、锁状态标志、线程持有的锁、偏向线程id、偏向时间戳。
     - 类型指针：指向类型元数据InstanceKlass，确认该对象所属的类型。
     - 如果是数组：还需要记录数组的长度。
   
   * 实例数据（存放对象的有效信息，包括代码中定义的和从父类中继承的。存储顺序有两方面影响）
     
     * 代码中的书写顺序
     
     * 虚拟机分配策略
       
       * 默认分配策略是：相同宽带的字段会被分配到一起存放。以这个条件为前提，父类的变量会在子类的之前。
       
       * 如果是HotSpot允许子类中较窄变量插入父类变量的空隙中，节省一点空间，可以配置下面参数
         
         ```java
         +XX:CompactFields:true//默认就是true
         ```
   
   * 对齐填充（不是必须的,起占位符的作用）
     
     * HotSpot虚拟机器自动内存管理系统要求，对象起始地址必须是8字节的整数倍，所以任何对象的大小都必须是8字节的整数倍。
     
     * 对象头的部分已经被精心设计成了8字节或16字节，而实例数据部分内容无法保证长度，不够8字节的地方，用对齐填充来补齐即可。

2. 对象头信息里面有哪些东西？
   
   1. 运行时元数据（Mark Word），包括哈希值（hashcode）、GC分代年龄、锁状态标志、线程持有的锁、偏向线程id、偏向时间戳。
   2. 类型指针：指向类型元数据InstanceKlass，确认该对象所属的类型。
   3. 如果是数组：还需要记录数组的长度。
...
<img src="../image/chapter09/0001.png">
...
<img src="../image/chapter09/0002.png">
...
<img src="../image/chapter09/0003.png">
...
<img src="../image/chapter09/0004.png">
...
<img src="../image/chapter09/0005.png">
...
<img src="../image/chapter09/0006.png">