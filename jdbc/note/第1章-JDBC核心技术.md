...
![1566741430592](./../image/1566741430592.png)
...
![1555575760234](./../image/1555575760234.png)

* 有了JDBC，Java程序访问数据库时是这样的

![1555575981203](./../image/1555575981203.png)

* 总结如下

![1566741692804](./../image/1566741692804.png)
...
![1565969323908](./../image/1565969323908.png)
...
- 在程序中不需要直接去访问实现了 Driver 接口的类，而是由驱动程序管理器类(java.sql.DriverManager)去调用这些Driver实现。
  ...
  - mySql的驱动8.0之前的驱动： **com.mysql.jdbc.Driver**
  - mySql的驱动8.0周的的驱动：**com.mysql.cj.jdbc.Driver**
- maven仓库地址
...
![1555576477107](./../image/1555576477107.png)
...
```
         ...
         *        jdbc:mysql://localhost:3306/test
         *        192.168.33.45
         *        jdbc:mysql://192.168.33.45/3306/test
         *        当前电脑的省略写法！ 注意：本机和端口3306
         *        jdbc:mysql://localhost:3306/test = jdbc:mysql:///test
```
...
```java
    public void getConnect1() throws SQLException {
        // //1.实例化Driver,方式一
        Driver driver = null;
...
//        Driver driver = (Driver) clazz.newInstance();
        ...
        //4.调用driver的connect()，获取连接，只能通过properties
...
//        Class.forName("com.mysql.cj.jdbc.Driver");
        ...
        //获取url连接，方式一
//        DriverManager.getConnection("jdbc:mysql://localhost:3306/test?user=root&password=HHXXttxs19");
        //获取url连接，方式二
//        DriverManager.getConnection("jdbc:mysql://localhost:3306/test","root","HHXXttxs19");
        //获取url连接，方式三
        Properties info = new Properties();
        info.setProperty("user", "root");
        info.setProperty("password", "HHXXttxs19");
        Connection connection = DriverManager.getConnection("jdbc:mysql://localhost:3306/test", info);
...
```java
    ...
    public  void testConnection5() throws Exception {
        //1.加载配置文件
        InputStream is = JdbcBaseTest.class.getClassLoader().getResourceAsStream("jdbc.properties");
...
- 在 java.sql 包中有 3 个接口分别定义了对数据库的调用的不同方式：
  - Statement：用于执行静态 SQL 语句并返回它所生成结果的对象，拼接SQL有SQL注入漏洞。 
  - PrepatedStatement：SQL 语句被预编译并存储在此对象中，可以使用此对象多次高效地执行该语句。
  - CallableStatement：用于执行 SQL 存储过程

![1566573842140](./../image/1566573842140.png)
...
![1566569819744](./../image/1566569819744.png)
...
* 使用PreparedStatement实现的查询操作可以替换Statement实现的查询操作，解决Statement拼串和SQL注入问题。
...
public class PreparedStatementCrudTest {
    ...
    public void insert() throws IOException, ClassNotFoundException, SQLException {
        ...
        //4.获取连接
        Connection conn = DriverManager.getConnection(url,user,password);
        String sql = "insert into user(user_name,password,nick_name) value(?,?,?)";
        PreparedStatement preparedStatement = conn.prepareStatement(sql);
        preparedStatement.setObject(1,"lisi");
        preparedStatement.setObject(2,"abcd123");
        preparedStatement.setObject(3,"李四");
        int i = preparedStatement.executeUpdate();
        System.out.println("执行成功，影响条数=="+i);
    ...
    public void update() throws IOException, ClassNotFoundException, SQLException {
        ...
        PreparedStatement preparedStatement = conn.prepareStatement(sql);
        preparedStatement.setObject(1,"李四狗");
...
![1555580152530](./../image/1555580152530.png)
...
![1555579494691](./../image/1555579494691.png)

### 3.4.3 获取自增序列的id值

* 通过 conn.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS);预编时，传入参数获取结果。
...
```java
    ...
    public void insert() throws IOException, ClassNotFoundException, SQLException {
        ...
        while (resultSet.next()){
            System.out.println("id="+resultSet.getObject(1));//批量插入就是多个值。
        }
        System.out.println("执行成功，影响条数=="+i);
        preparedStatement.close();
        conn.close();
...
![1555581069798](./../image/1555581069798.png)
...
- 数据库提供的4种事务隔离级别：
  ...
  ![1555586275271](./../image/1555586275271.png)
...
![1566726681515](./../image/1566726681515.png)

- 层次结构：

![1566745811244](./../image/1566745811244.png)

## 7.1 自主实现

* 通过jdbc工具类，以及线程变量获取连接。
* 通过反射获取值。
...
public abstract class BaseDao<T> {
     ...
     */
    public List<T> executeQuery(String sql,Object... params) throws SQLException, InstantiationException, IllegalAccessException, NoSuchFieldException {
        ...
        while (resultSet.next()) {
            T t = type.newInstance(); //调用类的无参构造函数实例化对象!
...
![1555593464033](./../image/1555593464033.png)

- **工作原理：**

![1555593598606](./../image/1555593598606.png)
...
![1555595163263](./../image/1555595163263.png)

![1555595198644](./../image/1555595198644.png)
